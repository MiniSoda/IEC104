...
iec104_class::iec104_class():
	t3_testfr(10),t2_supervisory(8),t1_startdtact(6)
{
    strncpy( slaveIP, "", 20 );
	...
	*/
    ...
    { // Control messages
        ...
        case STOPDTACT:
			...
			//mLog.pushMsg("-->  STOPDT激活");
            // only slave responds
            break;
            
        case STOPDTCON:
            mLog.pushMsg("--> STOPDTCON");
			//mLog.pushMsg("--> STOPDT确认");
            // do what?
            break;
            
        case TESTFRCON:
            mLog.pushMsg("--> TESTFRCON");
			//mLog.pushMsg("--> TESTFR确认");
            // do what?
            break;
    ...
    { // data message
		...
		case INTEGRATEDTOTALS://101:电能量
			if (papdu->asduh.cause==ACTCONFIRM)
			{
				 mLog.pushMsg("    INTEGRATEDTOTALS ACT CON ------------------------------------------------------------------------");
				//mLog.pushMsg("    遥脉总召 响应确认 ---------------------------------------");
				ITObjectCnt = 0;
